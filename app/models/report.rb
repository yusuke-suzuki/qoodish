...
class Report < ApplicationRecord
  MODERATABLE_TYPES = [Review.name, Comment.name, Map.name, Chapter.name, Journal.name, User.name].freeze
  ...
  def self.moderatable_scope(type, viewer)
    case type
    when Review.name
      Review.referenceable_by(viewer)
    when Comment.name
      Comment.visible.where(commentable_type: Review.name,
                            commentable_id: moderatable_scope(Review.name, viewer).select(:id))
    when Map.name
