...
class Me::CoauthorshipInvitationsControllerTest < ActionDispatch::IntegrationTest
  test 'index returns pending invitations received by the current user' do
    stub_google_auth(users(:me)) do
      get '/me/coauthorship_invitations', headers: { 'Authorization': 'Bearer dummytoken' }
    end
  ...
  test 'accept creates a coauthorship for the invitee' do
    ...
    stub_google_auth(users(:me)) do
      post "/me/coauthorship_invitations/#{invitation.id}/accept",
  ...
  test 'decline marks the invitation declined' do
    ...
    stub_google_auth(users(:me)) do
      post "/me/coauthorship_invitations/#{invitation.id}/decline",
           headers: { 'Authorization': 'Bearer dummytoken' }
    end
  ...
  test 'accept an invitation sent to another user should raise not found error' do
    invitation = coauthorship_invitations(:pending_you_to_me)
