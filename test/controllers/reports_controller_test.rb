...
class ReportsControllerTest < ActionDispatch::IntegrationTest
  test 'a signed-in user reports a pin' do
    stub_google_auth(users(:me)) do
      post '/reports',
           params: {
             moderatable_type: 'Review',
             moderatable_id: reviews(:public_you_one).id,
             category: 'harassment',
    ...
    assert_equal reviews(:public_you_one), report.moderatable
  ...
  test 'the report records the locale of the request' do
    stub_google_auth(users(:me)) do
      post '/reports',
           params: { moderatable_type: 'Review', moderatable_id: reviews(:public_you_one).id, category: 'spam' },
  ...
  test 'content the user cannot reference answers not found' do
    stub_google_auth(users(:me)) do
      post '/reports',
           params: { moderatable_type: 'Review', moderatable_id: reviews(:private_unfollowing_you).id,
                     category: 'spam' },
           headers: { 'Authorization': 'Bearer dummytoken' }
    end
  ...
  test 'a second report on the same content answers unprocessable' do
    ...
    stub_google_auth(users(:me)) do
      post '/reports',
           params: { moderatable_type: 'Review', moderatable_id: reviews(:public_you_one).id, category: 'hate' },
           headers: { 'Authorization': 'Bearer dummytoken' }
    end
  ...
  test 'reporting your own pin answers unprocessable' do
    stub_google_auth(users(:me)) do
      post '/reports',
           params: { moderatable_type: 'Review', moderatable_id: reviews(:public_one).id, category: 'spam' },
           headers: { 'Authorization': 'Bearer dummytoken' }
    end
