...
class ModerationDecisionTest < ActiveSupport::TestCase
  ...
  test 'keeping the content records the reason and tells the reporter' do
    ...
    assert_enqueued_emails 1 do
      decision = ModerationDecision.keep!(moderatable: reviews(:public_you_one), reason: 'Not spam.')
    end
  ...
  test 'removing the content hides it without destroying it' do
    review = reviews(:public_you_one)
    Report.create!(moderatable: review, reporter: users(:me), category: 'hate')

    decision = nil

    assert_enqueued_emails 2 do
      decision = ModerationDecision.remove!(moderatable: review, reason: 'Hate speech.')
    end

    assert decision.removed?
    assert_not_includes Review.public_open, review
    assert Review.exists?(review.id)
    assert_equal users(:you), decision.author
    assert_includes decision.content_snapshot, review.comment
  end

  test 'keeping content that was hidden brings it back' do
    review = reviews(:public_you_one)
    ModerationDecision.remove!(moderatable: review, reason: 'Hate speech.')

    assert_not_includes Review.public_open, review

    travel 1.minute
    ModerationDecision.keep!(moderatable: review, reason: 'Objection upheld.')

    assert_includes Review.public_open, review
  end

  test 'an account with hidden content can still be deleted' do
    ModerationDecision.remove!(moderatable: reviews(:public_you_one), reason: 'Hate speech.')

    stub_identity_platform { users(:you).destroy! }

    assert_not User.exists?(users(:you).id)
    assert_not Review.exists?(reviews(:public_you_one).id)
  end
  ...
  test 'hidden content can no longer be reported' do
    ...
    assert_raises(ActiveRecord::RecordNotFound) do
      Report.moderatable_for(Review.name, review.id, users(:me))
    end
  end
  ...
  test 'a report whose target is gone is closed as unavailable' do
    review = reviews(:public_you_one)
    report = Report.create!(moderatable: review, reporter: users(:me), category: 'spam')
  ...
  test 'two decisions sharing a timestamp answer different reports' do
    ...
    first = Report.create!(moderatable: reviews(:public_you_one), reporter: users(:me), category: 'spam',
                           created_at: now - 1.minute)
    earlier = ModerationDecision.create!(moderatable: reviews(:public_you_one), author: users(:you),
                                         outcome: 'kept', reason: 'Not spam.', created_at: now)
    second = Report.create!(moderatable: reviews(:public_you_one), reporter_email: 'guest@example.com',
                            category: 'hate', created_at: now)
    later = ModerationDecision.create!(moderatable: reviews(:public_you_one), author: users(:you),
  ...
  test 'a decision answers only the reports filed since the previous one' do
    ...
    second = Report.create!(moderatable: reviews(:public_you_one), reporter_email: 'guest@example.com',
                            category: 'hate')
    travel 1.minute
    later = ModerationDecision.keep!(moderatable: reviews(:public_you_one), reason: 'Still fine.')
  ...
  test 'a decision without a moderator is recorded as made by the system' do
    decision = ModerationDecision.keep!(moderatable: reviews(:public_you_one), reason: 'Automated check passed.')
